...
router.post('/api/app/upload', async (req, res) => {
  try {
    ...
    const filepath = data[0].filepath[0]

    {
      if (typeof authorization !== 'string') throw { error: new Error(), data: { code: 500 } }
      if (typeof filepath !== 'string') throw { error: new Error(), data: { code: 500 } }
    ...
    if (filepath.includes('../')) throw { error: new Error(), data: { code: 500 } }

    const pathComplete = path.join(__dirname, '../public', filepath)
    const pathParent = path.dirname(pathComplete)
    const pathExist = fs.existsSync(pathComplete)

    if (pathExist === true) fs.unlinkSync(pathComplete)

    fs.mkdirSync(pathParent, { recursive: true })
    fs.renameSync(file.filepath, pathComplete)
...
router.post('/api/app/upload/delete', async (req, res) => {
  ...
  try {
    ...
    if (pathExist === true) {
      const isFile = fs.statSync(pathComplete).isFile()
      const isDirectory = fs.statSync(pathComplete).isDirectory()
...
router.post('/api/app/upload/ls', async (req, res) => {
  const { authorization } = req.headers
  ...
  try {
    ...
    function walk(absoluteDir, relativeDir) {
      ...
      if (fs.existsSync(absoluteDir) === false) return children

      for (const name of fs.readdirSync(absoluteDir)) {
        const pathAbsolute = path.join(absoluteDir, name)
        const filepath = `${relativeDir}/${name}`.replace(/\/+/g, '/')
        const stat = fs.statSync(pathAbsolute)

        if (stat.isDirectory()) {
          children.push({ path: pathRelative, type: 'dir', children: walk(pathAbsolute, pathRelative) })
        }
        if (stat.isFile()) {
          children.push({ path: pathRelative, type: 'file', children: [] })
        }
      }
    ...
    res.send({ code: 200, data: { path: '/', type: 'dir', children: walk(publicDir, '') } })
  } catch (e) {
    if (process.argv.includes('--dev')) { console.log(e?.error || e) }; res.status(e?.data?.status || 500).send({ code: e?.data?.code || 500, message: e?.data?.message || '异常错误' })
  }
})
