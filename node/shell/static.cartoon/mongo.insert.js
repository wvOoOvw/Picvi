...
const _run = async () => {
  ...
  // 前置查询所有已存在的name
  const existingCartoons = await Mongo.client.db(Collection).collection('Cartoon').find({}, { projection: { name: 1 } }).toArray()
  const existingNames = existingCartoons.map(doc => doc.name)
  ...
  for (const dir of dirs) {
    ...
    const created = await Mongo.client.db(Collection).collection('Cartoon').insertOne({
      name: '',
      description: '',
      tag: [],
      actor: [],
