...
const deleteFolder = async (props) => {
  const deletepath = props.deletepath
...
const _run = async () => {
  // 前置一次性获取所有专辑数据
  const albums = await Mongo.client.db(Collection).collection('Album').find({ _id: 1, name: 1 }).toArray()
  const albumMap = new Map(albums.map(album => [album.name, album]))
  ...
  for (const dir of dirs) {
    ...
    if (!album) {
      console.log('album not found, skip', name)
      continue
    }

    const _id = String(album._id)

    console.log('process album', _id, name)

    // 删除远程文件夹
    const remoteFolder = `/album/${_id}`
    try {
    ...
    const description = `包含内容：${subscribeviewImage.length}P + ${subscribeviewVideo.length}V`
    ...
    console.log('update album', _id, 'subscribeview:', subscribeview.length, 'poster:', subscribeviewImage.length > 0 ? 1 : 0, 'preview:', subscribeviewImage.slice(0, 3).length)
  }
