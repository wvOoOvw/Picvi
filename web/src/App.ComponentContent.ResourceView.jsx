...
function ResourceViewPane(props) {
  ...
  const viewImageLink = React.useMemo(() => item.subscribeview || item.preview, [item.preview, item.subscribeview])
  ...
  const onFavorite = async () => {
    ...
    if (favorited === true) {
      await Fetch.json(`/api/app/user/update/${resource}/favorite`, { [`${resource}_id`]: item._id, favorite: false })
        .then(() => {
    ...
    if (favorited !== true) {
      await Fetch.json(`/api/app/user/update/${resource}/favorite`, { [`${resource}_id`]: item._id, favorite: true })
        .then(() => {
      ...
      <div style={{ width: paneStyleWidth[0], maxWidth: '100%', display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <Typography variant='body1' style={{ fontSize: 12, opacity: 0.5 }}>{title}ID：{item._id}</Typography>
...
function ResourceView(props) {
  ...
  const onFetch = async () => {
    ...
    await Fetch.json(`/api/app/${resource}/find`, { [`${resource}_id`]: params._id })
      .then(res => {
        setItem(res.data)
      })
      .catch(res => {
    ...
    await Fetch.json(`/api/app/${resource}/find/favorited`, { [`${resource}_id`]: params._id })
      .then(res => {
  ...
  const onFetchRecent = async () => {
    ...
    await Fetch.json(`/api/app/${resource}/find/list`, { filter: { status: 1, actor: item.actor }, seed: seed, skip: 0, limit: 8 })
      .then(res => {
        setRecent(res.data)
      })
  }
