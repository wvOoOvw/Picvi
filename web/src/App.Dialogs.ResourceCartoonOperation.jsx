...
function App() {
  ...
  const Component =
    <Dialog open={contextApp.dialogsArrayAction.exist('ResourceCartoonOperation')} onClose={() => contextApp.dialogsArrayAction.remove('ResourceCartoonOperation')} sx={{ '& .MuiDialog-paper': { width: 600, maxWidth: 'unset' } }}>
      ...
      <DialogContent style={{ paddingTop: 12 }}>
        {
          cartoonLoading !== true ?
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              {step === 0 ? <StepName value={cartoon} onChange={setCartoon} optionTag={optionTag} optionActor={optionActor} type="cartoon" /> : null}
              {step === 1 ? <StepDescription value={cartoon} onChange={setCartoon} optionTag={optionTag} optionActor={optionActor} type="cartoon" /> : null}
              {step === 2 ? <StepTag value={cartoon} onChange={setCartoon} optionTag={optionTag} optionActor={optionActor} type="cartoon" /> : null}
              {step === 3 ? <StepActor value={cartoon} onChange={setCartoon} optionTag={optionTag} optionActor={optionActor} type="cartoon" /> : null}
              {step === 4 ? <StepPoster value={cartoon} onChange={setCartoon} optionTag={optionTag} optionActor={optionActor} type="cartoon" /> : null}
              {step === 5 ? <StepPreview value={cartoon} onChange={setCartoon} optionTag={optionTag} optionActor={optionActor} type="cartoon" /> : null}
              {step === 6 ? <StepSubscribeview value={cartoon} onChange={setCartoon} optionTag={optionTag} optionActor={optionActor} type="cartoon" /> : null}
              {step === 7 ? <StepSetting value={cartoon} onChange={setCartoon} optionTag={optionTag} optionActor={optionActor} type="cartoon" /> : null}
            </div>
            : null
        }
        {
      ...
      <DialogActions>
        <Button onClick={onUploadFolder} disabled={cartoonLoading} style={{ whiteSpace: 'nowrap' }}>上传文件夹</Button>
