...
function App() {
  ...
  const onFavorite = async () => {
    ...
    if (favorited === true) {
      await Fetch.json('/api/app/user/update/album/favorite', { album_id: item._id, favorite: false })
        .then(() => {
    ...
    if (favorited !== true) {
      await Fetch.json('/api/app/user/update/album/favorite', { album_id: item._id, favorite: true })
        .then(() => {
  ...
  const onJump = async () => {
    contextApp.dialogsArrayAction.add('PageJump', { current: viewImageLinkIndex + 1, max: viewImageLink.length, onConfirm: (page) => setViewIndex(page - 1) })
  }
  ...
  const onFetchRecent = async () => {
    ...
    await Fetch.json('/api/app/album/find/list', { filter: { status: 1, actor: item.actor }, seed: seed, skip: 0, limit: 8 })
      .then(res => {
        setRecent(res.data)
      })
  }
                          ...
                          <div style={{ width: paneStyleWidth[0], maxWidth: '100%', display: 'flex', flexDirection: 'column', gap: 24 }}>
...

                              {
                                viewImageLink.length > 1 ?
                                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    ...
                                    <Typography color='primary' style={{ fontSize: 14 }} onClick={onJump}>{viewImageLinkIndex + 1} / {viewImageLink.length}</Typography>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 12 }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                <Typography variant='body1' style={{ fontSize: 12, opacity: 0.5 }}>图集ID：{item._id}</Typography>
