...
function App() {
  ...
  const onFetch = async () => {
    ...
    await Fetch.json('/api/app/video/find/list', { filter: { ...filter, latest: undefined }, seed: newSeed, skip: 0, limit: 10 })
      .then(res => {
        setList(res.data)
  ...
  const onFetchScroll = async () => {
    ...
    await Fetch.json('/api/app/video/find/list', { filter: { ...filter, latest: undefined }, seed: seed, skip: list.length, limit: 10 })
      .then(res => {
