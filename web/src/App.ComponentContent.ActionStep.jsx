...
export function StepName(props) {
  ...
  const Component =
    <>
      <TextField fullWidth autoComplete='off' label='名称' variant='standard' value={value.name} onChange={e => onChange({ ...value, name: e.target.value })} />
    </>
...
export function StepDescription(props) {
  ...
  const Component =
    <>
      <TextField fullWidth multiline autoComplete='off' label='描述' variant='standard' value={value.description} onChange={e => onChange({ ...value, description: e.target.value })} />
    </>
...
export function StepTag(props) {
  ...
  const onDelete = (tag) => {
    onChange(i => ({ ...i, tag: i.tag.filter(n => n !== tag) }))
  }
  ...
  const Component =
    <div>
      ...
      <div style={{ maxWidth: '100%', display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 16 }}>
        {
          optionTag.map(i => <Button key={i.tag} style={{ flexShrink: 0, minWidth: 'unset', padding: '4px 8px', fontSize: 12 }} variant='text' onClick={() => onChange(n => ({ ...n, tag: [...n.tag, i.tag] }))}>{i.tag}</Button>)
        }
      </div>
    </div>
...
export function StepActor(props) {
  ...
  const onDelete = (actor) => {
    onChange(i => ({ ...i, actor: i.actor.filter(n => n !== actor) }))
  }
  ...
  const onAdd = () => {
    ...
    onChange(i => ({ ...i, actor: [...i.actor, add] }))
    setAdd('')
  }
  ...
  const Component =
    <div>
      ...
      <div style={{ maxWidth: '100%', display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 16 }}>
        {
          optionActor.map(i => <Button key={i.actor} style={{ flexShrink: 0, minWidth: 'unset', padding: '4px 8px', fontSize: 12 }} variant='text' onClick={() => onChange(n => ({ ...n, actor: [...n.actor, i.actor] }))}>{i.actor}</Button>)
        }
      </div>
    </div>
        ...
        value._id !== undefined ?
          <>
            ...
            <MediaAction
              ...
              _id={value._id}
              value={value.poster}
              onChange={link => onChange(i => ({ ...i, poster: link }))}
              onChangeAppend={link => onChange(i => ({ ...i, poster: [...i.poster, link] }))}
            />
          </>
          : null
      }
        ...
        value._id !== undefined ?
          <>
            ...
            <MediaAction
              ...
              _id={value._id}
              value={value.preview}
              onChange={link => onChange(i => ({ ...i, preview: link }))}
              onChangeAppend={link => onChange(i => ({ ...i, preview: [...i.preview, link] }))}
            />
          </>
          : null
      }
...

      {
        value._id !== undefined ?
          <MediaAction
            ...
            onChange={link => onChange(i => ({ ...i, subscribeview: link }))}
            onChangeAppend={link => onChange(i => ({ ...i, subscribeview: [...i.subscribeview, link] }))}
          />
          : null
      }
    ...
    <>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <Switch checked={value.status === 1} onChange={(e, v) => onChange(({ ...value, status: v ? 1 : 0 }))} />
        </div>
      </div>
    </>
