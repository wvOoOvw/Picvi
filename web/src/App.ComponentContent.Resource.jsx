...
function Resource(props) {
  ...
  const onFetch = async () => {
    ...
    await Fetch.json(`/api/app/${resource}/find/list`, { filter: { ...filter, latest: undefined }, seed: newSeed, skip: 0, limit: 10 })
      .then(res => {
        setList(res.data)
  ...
  const onFetchScroll = async () => {
    ...
    await Fetch.json(`/api/app/${resource}/find/list`, { filter: { ...filter, latest: undefined }, seed: seed, skip: list.length, limit: 10 })
      .then(res => {
  ...
  const onEdit = (item) => {
    const onRefresh = async (item) => {
      contextApp.loadingArrayAction.add(`${resource}InformationEdit`)

      await Fetch.json(`/api/app/${resource}/find`, { [`${resource}_id`]: item._id })
        .then(res => {
      ...
      contextApp.loadingArrayAction.remove(`${resource}InformationEdit`)
    }

    contextApp.dialogsArrayAction.add(`${resource}InformationOperation`, { _id: item._id, onRefresh: async () => await onRefresh(item) })
  }

  const onDelete = (item) => {
    const onConfirm = async () => {
      contextApp.loadingArrayAction.add(`${resource}InformationDelete`)

      await Fetch.json(`/api/app/admin/${resource}/delete`, { [`${resource}_id`]: item._id })
        .then(res => {
      ...
      contextApp.loadingArrayAction.remove(`${resource}InformationDelete`)
      ...
      <div style={{ width: '100%', height: '100%', display: 'flex', justifyContent: 'center', padding: 24, paddingBottom: 96 }}>
        <div style={{ width: '100%', maxWidth: 1200, height: 'fit-content' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, marginBottom: 20 }}>
            <Typography color='primary' style={{ fontSize: 28 }}>{title}</Typography>
                    ...
                    return <div>
                      {
                        breakpointCols ?
                          <PosterCardMasonry
                            ...
                            cards={list}
                            onClickCard={(card) => navigate(`/${resource}/${card._id}`)}
