...
function VideoView(props) {
  ...
  const videoFavorited = props.videoFavorited
  const videoRecent = props.videoRecent
  ...
  const onFavorite = async () => {
    ...
    if (videoFavorited === true) {
      await Fetch.json('/api/app/user/update/video/favorite', { video_id: video._id, favorite: false })
        .then(() => {
    ...
    if (videoFavorited !== true) {
      await Fetch.json('/api/app/user/update/video/favorite', { video_id: video._id, favorite: true })
        .then(() => {
          ...
          <Paper style={{ width: '100%', borderRadius: '0px 0px 24px 24px', overflow: 'hidden', position: 'relative' }}>
            {
              viewImageLink.length > 0 ?
                <>
                  ...
                  />
                  {
                    video.subscribeview === undefined ? <Button fullWidth variant='contained' component='div' color='inherit' style={{ minWidth: 'unset', padding: '8px 12px', fontSize: 14, opacity: 0.75, position: 'absolute', bottom: 0 }} onClick={onSubscribe}>查看全部内容</Button> : null
                  }
                </>
                : null
            }
...
function App() {
  ...
  const [videoFavorited, setVideoFavorited] = React.useState()
  const [videoRecent, setVideoRecent] = React.useState([])
  ...
  const onFetchVideoRefresh = async () => {
    await Fetch.json('/api/app/video/find', { video_id: params._id })
      .then(res => {
        setVideo(res.data)
      })
  ...
  const onFetchVideoRecent = async () => {
    const seed = Math.round(Math.random() * 10000 + 1)

    await Fetch.json('/api/app/video/find/list', { filter: { status: 1, actor: video.actor }, seed: seed, skip: 0, limit: 8 })
      .then(res => {
        setVideoRecent(res.data)
      })
  }

  React.useEffect(() => { onFetchVideo() }, [params._id])
  React.useEffect(() => { if (video && videoRecent.length === 0) onFetchVideoRecent() }, [video])

  ReactActivation.useActivate(() => { onFetchVideoRefresh() })
    ...
    <>
      {
        videoLoading !== true ?
          <>
            {
              video !== undefined && video._id === params._id ?
                <>
                  {
                    video.status === 1 ?
                      <>
                        <VideoView video={video} videoFavorited={videoFavorited} videoRecent={videoRecent} onRefresh={onFetchVideoRefresh} />
                      </>
                      : null
                  }
                  {
