...
function App() {
  ...
  const onFavorite = async () => {
    ...
    if (favorited === true) {
      await Fetch.json('/api/app/user/update/cartoon/favorite', { cartoon_id: item._id, favorite: false })
        .then(() => {
    ...
    if (favorited !== true) {
      await Fetch.json('/api/app/user/update/cartoon/favorite', { cartoon_id: item._id, favorite: true })
        .then(() => {
  ...
  const onFetchRecent = async () => {
    ...
    await Fetch.json('/api/app/cartoon/find/list', { filter: { status: 1, actor: item.actor }, seed: seed, skip: 0, limit: 8 })
      .then(res => {
        setRecent(res.data)
      })
  }
                          ...
                          <div style={{ width: paneStyleWidth[0], maxWidth: '100%', display: 'flex', flexDirection: 'column', gap: 24 }}>
                            ...
                            <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 12 }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                <Typography variant='body1' style={{ fontSize: 12, opacity: 0.5 }}>漫画ID：{item._id}</Typography>
