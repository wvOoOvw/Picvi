...
function MediaLoading(props) {
  const Component =
    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', gap: 8, ...props.style }}>
      ...
      <Typography variant='body2' style={{ fontSize: 12, color: 'black', opacity: 0.5 }}>
        {props.extensionMode === 'Image' ? '加载图片中' : null}
        {props.extensionMode === 'Video' ? '加载视频中' : null}
      </Typography>
    </div>
...
function MediaContent(props) {
  ...
  const Loading =
    <>
      {
        props.loading ? <MediaLoading style={{ width: '100%', height: '100%', position: 'absolute', top: 0, left: 0, background: 'rgba(255, 255, 255, 1)' }} loadingSize={props.loadingSize} extensionMode={props.extensionMode} /> : null
      }
    </>
...
function Media(props) {
  ...
  const Component =
    <>
      ...
      {
        loading === true && mediaSrcPrevious === undefined ? <MediaLoading loadingSize={loadingSize} extensionMode={extensionMode} style={{ ...style, ...styleMediaInvisible, ...styleMediaLoading }} /> : null
      }
      {
