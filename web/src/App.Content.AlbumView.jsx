...
function AlbumView(props) {
  ...
  const albumFavorited = props.albumFavorited
  ...
  const viewImageLink = React.useMemo(() => album.subscribeview || album.preview, [album.preview, album.subscribeview])
      ...
      <div style={{ width: paneStyleWidth[0], maxWidth: '100%', display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          <Paper style={{ width: '100%', borderRadius: '0px 0px 24px 24px', overflow: 'hidden', position: 'relative' }}>
            {
              viewImageLink.length > 0 ?
                <>
                  ...
                  />
                  {
                    album.subscribeview === undefined ? <Button fullWidth variant='contained' component='div' color='inherit' style={{ minWidth: 'unset', padding: '8px 12px', fontSize: 14, opacity: 0.75, position: 'absolute', bottom: 0 }} onClick={onSubscribe}>查看全部内容</Button> : null
                  }
                </>
                : null
            }
        ...
        <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <Typography variant='body1' style={{ fontSize: 12, opacity: 0.5 }}>图集ID：{album._id}</Typography>
            <CopyAllIcon style={{ width: 12, height: 12, cursor: 'pointer' }} onClick={() => onCopy(album._id)} />
          </div>

          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12 }}>
            <Typography color='primary' variant='body1' style={{ fontSize: 20 }}>{album.name}</Typography>
            {
              favoriteLoading !== true && albumFavorited === true ? <Button variant='text' color='primary' style={{ minWidth: 'unset', padding: 4, color: 'gray', height: 'fit-content' }} onClick={onFavorite}><StarIcon color='primary' style={{ width: 24, height: 24 }} /></Button> : null
            }
            {
              favoriteLoading !== true && albumFavorited !== true ? <Button variant='text' color='primary' style={{ minWidth: 'unset', padding: 4, color: 'gray', height: 'fit-content' }} onClick={onFavorite}><StarBorderIcon color='primary' style={{ width: 24, height: 24 }} /></Button> : null
            }
            {
          ...
          <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 8 }}>
            {
              album.description ? <Typography color='primary' style={{ whiteSpace: 'pre-wrap', wordWrap: 'break-word', fontSize: 14 }}>{album.description}</Typography> : null
            }
            {
...
function App() {
  ...
  const [albumFavorited, setAlbumFavorited] = React.useState()
  ...
  const onFetchAlbum = async () => {
    ...
    await Fetch.json('/api/app/album/find/favorited', { album_id: params._id })
      .then(res => {
        setAlbumFavorited(res.data)
      })
      .catch(res => {
        contextApp.messageArrayAction.add('查询错误')
      })
  ...
  const onFetchAlbumRecent = async () => {
    ...
    await Fetch.json('/api/app/album/find/list', { filter: { status: 1, actor: album.actor }, seed: seed, skip: 0, limit: 8 })
      .then(res => {
  ...
  React.useEffect(() => { if (album && albumRecent.length === 0) onFetchAlbumRecent() }, [album])
                ...
                <>
                  {
                    album.status === 1 ?
                      <>
                        <AlbumView album={album} albumFavorited={albumFavorited} albumRecent={albumRecent} onRefresh={onFetchAlbumRefresh} />
                      </>
                      : null
                  }
                  {
