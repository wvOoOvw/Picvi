...
function App() {
  ...
  const Component =
    <Dialog open={contextApp.dialogsArrayAction.exist('ResourceVideoOperation')} onClose={() => contextApp.dialogsArrayAction.remove('ResourceVideoOperation')} sx={{ '& .MuiDialog-paper': { width: 600, maxWidth: 'unset' } }}>
      ...
      <DialogContent style={{ paddingTop: 12 }}>
        {
          videoLoading !== true ?
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              {step === 0 ? <StepName value={video} onChange={setVideo} optionTag={optionTag} optionActor={optionActor} type="video" /> : null}
              {step === 1 ? <StepDescription value={video} onChange={setVideo} optionTag={optionTag} optionActor={optionActor} type="video" /> : null}
              {step === 2 ? <StepTag value={video} onChange={setVideo} optionTag={optionTag} optionActor={optionActor} type="video" /> : null}
              {step === 3 ? <StepActor value={video} onChange={setVideo} optionTag={optionTag} optionActor={optionActor} type="video" /> : null}
              {step === 4 ? <StepPoster value={video} onChange={setVideo} optionTag={optionTag} optionActor={optionActor} type="video" /> : null}
              {step === 5 ? <StepPreview value={video} onChange={setVideo} optionTag={optionTag} optionActor={optionActor} type="video" /> : null}
              {step === 6 ? <StepSubscribeview value={video} onChange={setVideo} optionTag={optionTag} optionActor={optionActor} type="video" /> : null}
              {step === 7 ? <StepSetting value={video} onChange={setVideo} optionTag={optionTag} optionActor={optionActor} type="video" /> : null}
            </div>
            : null
        }
        {
      ...
      <DialogActions>
        <Button onClick={onUploadFolder} disabled={videoLoading} style={{ whiteSpace: 'nowrap' }}>上传文件夹</Button>
