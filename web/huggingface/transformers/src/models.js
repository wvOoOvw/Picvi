...
export class PreTrainedModel extends Callable {
    ...
    */
    ...
    async generate({
        ...
        streamer = null,
        request_idle_callback_use = false,
        request_idle_callback_remain = 0,
        ...
        while (whileContinue) {
            ...
            if (request_idle_callback_use !== true || window?.requestIdleCallback === undefined) {
                await run()
            }
            if (request_idle_callback_use === true && window?.requestIdleCallback !== undefined) {
                await new Promise(resolve => {
                    window.requestIdleCallback(async (idle) => {
                        while (whileContinue && idle.timeRemaining() > request_idle_callback_remain) {
                            await run()
                        }
                        resolve()
                    })
